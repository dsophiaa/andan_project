...
Далее мы поняли, что наблюдений недостаточно для построения более-менее адекватной модели, поэтому было принято решение расширить датасет, собрав информацию из doc файлов с Росстата. Их сбор и предобработка, а также EDA итогового датасета лежит в файле [pt_2_Предобработка2_EDA.ipynb](pt_2_Предобработка2_EDA.ipynb)
